...
| **Протокол**     | `ip` (любой), `tcp`, `udp`, `icmp`                               | `ip`, `tcp`, `udp`             |
...
При наличии нескольких пулов на фильтре трафик может совпасть с ACL **разных пулов**. В этом случае пакет попадёт в пул с **наивысшим приоритетом** — с наименьшим значением `priority`: пулы проверяются по возрастанию этого значения.

| Параметр      | Описание                                                    |
| ------------- | ----------------------------------------------------------- |
| **priority**  | Числовое значение приоритета пула (чем меньше — тем раньше проверяется пул) |
