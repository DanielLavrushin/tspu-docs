...
> **Примечание:** формат секции DPI различается между пилотным проектом (Урал) и федеральным проектом. В пилотном проекте настройки реестра Роскомнадзора находятся внутри DPI-листа 0. В федеральном проекте они вынесены в отдельную секцию, а все DPI-листы (0–16) стали абсолютно идентичными по структуре.
...
На фильтре настраиваются DPI-листы с номерами **0–16** — лист 0 и шестнадцать листов 1–16. В более поздних версиях ПО листы создаются по мере необходимости командой `create dpilist N` (по документации производителя — в стандартной конфигурации с номерами 0–24, по запросу заказчика до 1000).
...
| Значение     | Описание                                                 | Применение в ТСПУ            |
| ------------ | -------------------------------------------------------- | ---------------------------- |
| **block**    | HTTP — перенаправление на заглушку, HTTPS — TCP Reset    | Основной рабочий режим       |
