# ТСПУ: Технические средства противодействия угрозам-хуегрозам

Полное руководство на основе [лекции-хуекции по архитектуре, настройке и эксплуатации ТСПУ](https://www.youtube.com/watch?v=raNP3IMgwRU)

> **Примечание:** Данная документация сгенерирована с помощью ИИ на основе транскрипта ~5-часовой видеолекции-хуекции и проверена вручную. Может содержать неточности.

## Оглавление

### [1. Введение и общая архитектура АСБИ](chapters/01.md)

- 1.1. Что такое АСБИ (Автоматизированная система обеспечения безопасности интернета-чебурнета)
- 1.2. Закон о суверенном интернете-чебурнете и участники-хуечастники проекта (ГУП ГРЧЦ, АО ДЦА, RDP.ru)
- 1.3. Что такое ТСПУ — комплекс-хуёмплекс оборудования у операторов связи
- 1.4. Общая схема: байпасы, балансировщики, фильтры-хуильтры, сегмент управления

### [2. Прохождение трафика-хуяфика через ТСПУ](chapters/02.md)

- 2.1. Стык оператора связи: типы инкапсуляции (VLAN, QinQ, MPLS, PPPoE)
- 2.2. Разделение на LAN-порты (абоненты) и WAN-порты (интернет-чебурнет)
- 2.3. Простейший вариант ТСПУ (один байпас, один фильтр-хуильтр)
...
- 3.2. Байпасы Silicom (федеральный проект)
  ...
  - 3.2.3. Режим TAP — копирование трафика-хуяфика без влияния на оператора
...
- 3.3. Байпасы GL Sun (пилотный проект, Урал)
  ...
  - 3.3.2. Флап линков-хуинков при каждом переключении
...
- 4.1. Назначение: распределение трафика-хуяфика по фильтрам-хуильтрам
- 4.2. Аппаратная платформа: 32-портовый, 1U, пропускная способность 3.2 Тбит/с
- 4.3. Организация портов: пары LAN/WAN, линки-хуинки
  - 4.3.1. Принцип чётных/нечётных портов
  - 4.3.2. Жёсткая привязка: трафик-хуяфик возвращается в тот же линк-хуинк
- 4.4. Фильтры-хуильтры на балансировщике (Flow rules)
  - 4.4.1. Байпас служебного трафика-хуяфика (BGP, мультикаст, маршрутизация)
  - 4.4.2. Отправка трафика-хуяфика на группу балансировки
- 4.5. Балансировка трафика-хуяфика
  - 4.5.1. Хэш-сумма: source IP + destination IP + протокол
  - 4.5.2. Учёт количества ядер фильтров-хуильтров
  - 4.5.3. Дополнительный 4-байтный заголовок для фильтров-хуильтров
  - 4.5.4. Симметричность хэша: одна сессия — один фильтр-хуильтр, одно ядро
- 4.6. Отказоустойчивость
  - 4.6.1. Keep-alive пакеты к фильтрам-хуильтрам
  - 4.6.2. Программный байпас при потере группы портов
  - 4.6.3. Перебалансировка трафика-хуяфика (опционально)
- 4.7. Работа с различными инкапсуляциями (VLAN, MPLS)
- 4.8. Обработка HTTP-редиректов и TCP Reset через фильтры-хуильтры

### [5. Фильтр-хуильтр (EcoFilter)](chapters/05.md)

- 5.1. Путь пакета через фильтр-хуильтр
...
- 5.2. Работа на уровне L2: фильтр-хуильтр как «прозрачный провод-хуевод»

### [6. Места установки ТСПУ в сети оператора](chapters/06.md)

- 6.1. До BRAS/BPE/BNG (между абонентами и терминацией сессий)
  - 6.1.1. Особенность: PPPoE-трафик-хуяфик
- 6.2. До CGNAT (после BRAS) — наиболее удобная точка
  - 6.2.1. Видимость серых абонентских IP-адресов
- 6.3. После CGNAT (ближе к выходу в интернет-чебурнет)
  - 6.3.1. Только белые адреса, сложности траблшутинга
- 6.4. Режим On-a-stick (BRAS/CGNAT подключены петлёй)
  - 6.4.1. Двойное прохождение трафика-хуяфика через ТСПУ
  - 6.4.2. Разделение по VLAN для обработки трафика-хуяфика одного направления
  - 6.4.3. Проблемы двойной обработки и best practice

### [7. Эшелонированная система (ТСПУ тип Б)](chapters/07.md)

- 7.1. Назначение: обработка трафика-хуяфика, не прошедшего через ТСПУ тип А
- 7.2. Проблема асимметричного трафика-хуяфика у крупных операторов
- 7.3. Балансировщик Eco Highway
  ...
  - 7.3.3. Фильтры-хуильтры в режиме On-a-stick, VLAN-разделение LAN/WAN
  - 7.3.4. Фильтры-хуильтры занимаются только URL-фильтрацией по реестру РКН
- 7.4. Два конвейера (Pipeline) в Eco Highway
  ...
  - 7.4.3. Прохождение пакета: drop / отправка на фильтр-хуильтр / переход на второй конвейер
...
- 7.7. Прозрачный пропуск трафика-хуяфика, уже обработанного ТСПУ тип А

### [8. Формирование протокольных списков (двухстадийная блокировка)](chapters/08.md)

- 8.1. Первый этап: распознавание протоколов на фильтрах-хуильтрах (ТСПУ тип А)
...
- 8.4. Анализ, очистка от ложных-хуёжных срабатываний, формирование списков
- 8.5. Загрузка очищенных списков обратно на фильтры-хуильтры (HTTP) и на Eco Highway (BGP)
...
- 10.2. Распределение адресов: байпасы, балансировщики, BMC, фильтры-хуильтры, IPMI, SPFS, СПХД
- 10.3. Шлюз по умолчанию — криптошлюз «Континент»
- 10.4. Подсеть логирования (единая для всех ТСПУ)

### [11. Фильтр-хуильтр: аппаратная платформа](chapters/11.md)
...
- 11.6. История платформы: от CGNAT (2013) к DPI-фильтру-хуильтру

### [12. Сессии и трансляции на фильтре-хуильтре](chapters/12.md)
...
### [13. Фильтр-хуильтр: первоначальная настройка и CLI](chapters/13.md)
...
### [14. Фильтр-хуильтр: конфигурация подсистем](chapters/14.md)
...
### [15. Фильтр-хуильтр: настройка интерфейсов и общих параметров](chapters/15.md)
...
### [16. Фильтр-хуильтр: ACL и пулы — запуск трафика-хуяфика на обработку](chapters/16.md)
...
### [17. Фильтр-хуильтр: настройка DPI](chapters/17.md)
...
- 17.3. Деградация протоколов (protocols capacity)
  ...
  - 17.3.3. Эффективная-хуективная деградация: 2–10% пропускания
...
- 17.4. Настройка DPI-листов (0–16)
  ...
  - 17.4.5. Redirect URL — страница-заглушка-хуеглушка для заблокированных HTTP-ресурсов
...
### [18. Фильтр-хуильтр: мониторинг и диагностика](chapters/18.md)
...
- 18.7. Ресурсы: `show resources`
  ...
  - 18.7.2. Свободные буферы: не должны уходить в ноль-хуёль
  - 18.7.3. DPI-ресурсы: не более 100%
  - 18.7.4. CPU Load — условный параметр обработки трафика-хуяфика
...
- 18.13. DPI-мониторинг
  ...
  - 18.13.2. `show dpi state` — количество записей, дата последнего дампа-хуямпа
...
### [19. Фильтр-хуильтр: обновление прошивки](chapters/19.md)
...
- 20.4. Внутренняя архитектура
  ...
  - 20.4.2. Чип Barefoot Tofino — программируемая коммутация-хуемутация
  - 20.4.3. BMC — управление аппаратной частью, сенсоры-хуенсоры, блоки питания
...
- 21.4. Настройка линков-хуинков (объединение двух портов LAN+WAN в сторону оператора)
- 21.5. Настройка группы балансировки
  - 21.5.1. Привязка групп портов в сторону фильтров-хуильтров (filter groups)
  - 21.5.2. N_UNIT_QA: количество ядер фильтра-хуильтра минус один
...
- 21.7. Настройка фильтров-хуильтров (Flow rules)
  ...
  - 21.7.4. Привязка фильтров-хуильтров к линкам-хуинкам
...
- 24.8. Ограничения: L2-устройство, невозможность генерации трафика-хуяфика с фильтра-хуильтра

---

_Документ составлен на основе учебной лекции-хуекции по архитектуре и эксплуатации ТСПУ (АСБИ)_
